...
class PINN_Net(nn.Module):
    ...
    def data_mse_without_p(self, x, y, u, v):
        predict_out = self.forward(x, y)
        u_predict = predict_out[:, 0].reshape(-1, 1)
        v_predict = predict_out[:, 1].reshape(-1, 1)
        mse = torch.nn.MSELoss()
    ...
    def equation_mse_dimensionless_psi(self, x, y, Re):
        # 正问题
        predict_out = self.forward(x, y)
        # 获得预测的输出psi,p
        psi = predict_out[:, 0].reshape(-1, 1)
        ...
        # 计算偏微分方程的残差
        f_equation_x = (u * u_x + v * u_y) + p_x - 1.0 / Re * (u_xx + u_yy)
        f_equation_y = (u * v_x + v * v_y) + p_y - 1.0 / Re * (v_xx + v_yy)
        mse = torch.nn.MSELoss()
    ...
    def equation_mse_dimensionless(self, x, y, Re):
        # 正问题
        predict_out = self.forward(x, y)
...
def shuffle_data(x, y, u, v):
    X_total = torch.cat([x, y, u, v], 1)
