...
def read_3D_data_surround(filename):
    # 读取原始数据
    data_mat = scipy.io.loadmat(filename)
    stack = data_mat['stack']  # N*4 (x,y,u,v)
    x = stack[:, 0].reshape(-1, 1)
    ...
    feature_mat[0, :] = np.min(temp, 0)
    feature_mat[1, :] = np.max(temp, 0)
    right_index = (np.where(x == feature_mat[1, 0]))[0].reshape(-1, 1)
    left_index = (np.where(x == feature_mat[0, 0]))[0].reshape(-1, 1)
    front_index = (np.where(y == feature_mat[1, 1]))[0].reshape(-1, 1)
    back_index = (np.where(y == feature_mat[0, 1]))[0].reshape(-1, 1)
...
def read_2D_data_without_p_alone_line(filename, axis, num_of_line):
    # 读取原始数据
    data_mat = scipy.io.loadmat(filename)
    stack = data_mat['stack']  # N*4 (x,y,u,v)
    x = stack[:, 0].reshape(-1, 1)
    ...
    temp = np.concatenate((x, y, u, v), 1)
    feature_mat = np.empty((2, 4))
    feature_mat[0, :] = np.max(temp, 0)
    feature_mat[1, :] = np.min(temp, 0)
    if axis == 'x':
        x_uniq = np.unique(x).reshape(-1, 1)
        select_x = x_uniq[num_of_line, 0]
        index = np.where(x==select_x)[0].reshape(-1, 1)
        x = x[index].reshape(-1, 1)
        y = y[index].reshape(-1, 1)
        u = u[index].reshape(-1, 1)
        v = v[index].reshape(-1, 1)
        x = torch.tensor(x, dtype=torch.float32)
        y = torch.tensor(y, dtype=torch.float32)
        u = torch.tensor(u, dtype=torch.float32)
        v = torch.tensor(v, dtype=torch.float32)
        feature_mat = torch.tensor(feature_mat, dtype=torch.float32)
        return x, y, u, v, feature_mat
    elif axis == 'y':
        y_uniq = np.unique(y).reshape(-1, 1)
        select_y = y_uniq[num_of_line, 0]
        index = np.where(y == select_y)[0].reshape(-1, 1)
        x = x[index].reshape(-1, 1)
        y = y[index].reshape(-1, 1)
        u = u[index].reshape(-1, 1)
        v = v[index].reshape(-1, 1)
        ...
        print('wrong wrong wrong name of axis')
    return x, y, u, v, feature_mat
